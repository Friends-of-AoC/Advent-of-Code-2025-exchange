...
(defn main [& args]
  # (def lines (get-lines "y25day08_input.txt")) (def max 1000)
  (def lines (get-lines "y25day08_demo.txt")) (def max 10)
  ...
  (each [[box1 box2] _dist] closest-pairs
    ...
    (def same-circuit
      (and
        ...
        (= circuit1 circuit2)))
    (when (not same-circuit)
      (def new-circuit @[;circuit1 ;circuit2])

      (prin "New circuit ")
      (print (string/join (map box-to-str new-circuit) "<->"))
  ...
  (var result 1)
  (each circuit (take 3 (sort circuits (fn [a b] (> (length a) (length b)))))
    (pp circuit)
    (print (length circuit))
    (*= result (length circuit)))
