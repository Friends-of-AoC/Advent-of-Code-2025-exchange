...
for i, l in enumerate(matrix):
    if 'S' in l:
        ...
        if r1[0]==r2[0]: new_S = '-'
        if r1[1]==r2[1]: new_S = '|'
        if r1[0]<si and sj<r2[1]: new_S = 'L'
        if r1[0]<si and sj>r2[1]: new_S = '7'
        if r1[0]>si and sj<r2[1]: new_S = 'F'
        if r1[0]>si and sj>r2[1]: new_S = 'J'
        break

# copy the pipe loop into a new matrix, replace S by new_S
markings = [['.']*ncols for i in range(nrows)]
